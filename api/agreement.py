...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            item = quote_cloud.find_agreement(quote_cloud.load_agreements(), qid)
            if not item:
    ...
    def do_POST(self):
        try:
            ...
            if not quote_cloud.cloud_enabled():
                return _send(self, 503, {"ok": False, "error": "Home browser save", "home": True})
            saved = quote_cloud.save_agreement_item(_read_json(self))
    ...
    def do_DELETE(self):
        try:
            ...
            if not quote_cloud.delete_agreement(qid):
                return _send(self, 404, {"ok": False, "error": "Not found"})
